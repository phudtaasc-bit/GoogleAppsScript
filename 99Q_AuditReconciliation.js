 ...
 * Audit phân tầng, xác định đúng sheet/module gây sai lệch.
 *
 * Tầng 1: nguồn chi tiết Sheet 02 -> 03 -> 04.
 * Tầng 2: Sheet 04A đối chiếu trực tiếp với Sheet 04.
 * Tầng 3: Sheet 00 đối chiếu theo tên chỉ tiêu, không phụ thuộc số dòng.
 * Tầng 4: các phương trình FCFF, FCFE và chênh lệch phạm vi.
 *************************************************/
...
function FS99Q_chayAuditReconciliation() {
  ...
  info('TẦNG 1 - CẤU TRÚC', 'Dòng tiêu đề Sheet 02', meta02.headerRow,
    'Phát hiện tự động; không yêu cầu các sheet dùng cùng một dòng tiêu đề.');
  ...
  add('TẦNG 1 - DOANH THU', 'Sheet 02: Tổng doanh thu trước VAT = bán + thuê',
    ...
    'Kiểm tra nội tại Sheet 02.',
    'Sửa hàm lập Sheet 02 tại phần tổng doanh thu.', TOL_DONG);

  add('TẦNG 1 - DOANH THU', 'Dòng tiền khách hàng Sheet 02 = Sheet 03',
    sum02('Dòng tiền huy động từ KH'),
    sum03('Dòng tiền huy động từ KH'),
    'Kiểm tra luồng dữ liệu 02 -> 03.',
    'Sửa hàm cập nhật nguồn Sheet 03 hoặc cột Dòng tiền huy động từ KH.', TOL_DONG);

  const costNames = [
    'Chi XD/TB/khác trước VAT',
    'Chi GPMB trước VAT',
    'Tiền SDĐ/thuê đất trước VAT',
    'Chi HTKT trước VAT',
    'Chi phí bán hàng trước VAT',
    'Chi phí dự phòng trước VAT',
  ...
  add('TẦNG 1 - CHI PHÍ', 'Sheet 03: Tổng chi sau VAT = trước VAT + VAT đầu vào',
    sum03('Tổng chi sau VAT'),
    sum03('Tổng chi trước VAT') + sum03('VAT đầu vào'),
  ...
  add('TẦNG 1 - DÒNG TIỀN', 'FCFF Sheet 04 = Dòng tiền trước tài trợ',
    ...
    'FCFF giữ nguyên logic đã chốt.',
    'Sửa công thức FCFF_TIPV tại Sheet 04.', TOL_DONG);
  ...
  add('TẦNG 1 - DÒNG TIỀN', 'FCFE = FCFF + giải ngân vay - trả gốc',
    ...
    'Quan hệ FCFE đã chốt.',
    'Sửa công thức FCFE khả dụng cho CSH tại Sheet 04.', TOL_DONG);
  ...
  if (sh04A) {
    ...
    FS99Q_addAnnualSourceCheck_(add, annual, 'Dòng tiền huy động từ khách hàng',
      sum04('Dòng tiền huy động từ KH') / 1e9, 'Dòng tiền khách hàng', TOL_TY);
    FS99Q_addAnnualSourceCheck_(add, annual, 'Dòng tiền vốn CSH',
      sum04('Tổng dòng CSH vào dự án') / 1e9, 'Vốn CSH', TOL_TY);
    FS99Q_addAnnualSourceCheck_(add, annual, 'Dòng tiền vốn vay',
      sum04('Giải ngân vay') / 1e9, 'Giải ngân vay', TOL_TY);
    FS99Q_addAnnualSourceCheck_(add, annual, 'VAT phải nộp',
      sum04('VAT phải nộp') / 1e9, 'VAT phải nộp', TOL_TY);
    FS99Q_addAnnualSourceCheck_(add, annual, 'Thuế TNDN',
      sum04('Thuế TNDN') / 1e9, 'Thuế TNDN', TOL_TY);
    FS99Q_addAnnualSourceCheck_(add, annual, 'Lãi vay',
      sum04('Lãi vay vốn hóa') / 1e9, 'Lãi vay', TOL_TY);
    FS99Q_addAnnualSourceCheck_(add, annual, 'Trả gốc vay',
      sum04('Trả gốc') / 1e9, 'Trả gốc', TOL_TY);
    FS99Q_addAnnualSourceCheck_(add, annual, 'FCFF dự án',
      sum04('FCFF_TIPV') / 1e9, 'FCFF', TOL_TY);
    FS99Q_addAnnualSourceCheck_(add, annual, 'FCFE vốn CSH',
      sum04('FCFE khả dụng cho CSH') / 1e9, 'FCFE', TOL_TY);
    ...
    add('TẦNG 2 - 04A', 'Dòng tiền thuần sau tài trợ = Tổng vào - Tổng ra',
      ...
      'Quan hệ được kiểm tra ngay trên Sheet 04A.',
      'Sửa hàm FS04A_buildAnnualFromSheet04_: dòng tổng vào, tổng ra hoặc dòng tiền thuần.', TOL_TY);
    ...
    add('TẦNG 2 - 04A', 'FCFF = Thuần sau tài trợ - CSH - Vay + Trả gốc + Lãi vay',
      ...
      'Kiểm tra cầu nối từ dòng tiền vào/ra đến FCFF.',
      'Sửa công thức phần C Sheet 04A.', TOL_TY);

    add('TẦNG 2 - 04A', 'FCFE = Thuần sau tài trợ - CSH + Lãi vay',
      fcfeA, net - equity + interest,
      'Kiểm tra cầu nối từ dòng tiền vào/ra đến FCFE.',
      'Sửa công thức phần C Sheet 04A.', TOL_TY);
  } else {
  ...
  // ================================================================
  // TẦNG 3 - SHEET 00 LINH HOẠT THEO TÊN CHỈ TIÊU
  // ================================================================
  ...
  FS99Q_addSummaryCheck_(add, summary, 'Tổng doanh thu có VAT', srcRevenueTy,
    'Tổng doanh thu có VAT', 'Sửa module lập mục III Sheet 00: chỉ tiêu Tổng doanh thu có VAT.', TOL_TY);
  FS99Q_addSummaryCheck_(add, summary, 'Tổng chi phí có VAT', srcCostTy,
    'Tổng chi phí sau VAT + lãi vay', 'Sửa module lập mục III Sheet 00: chỉ tiêu Tổng chi phí có VAT.', TOL_TY);
  FS99Q_addSummaryCheck_(add, summary, 'Lợi nhuận sau thuế', srcProfitTy,
    'Lợi nhuận sau thuế', 'Sửa module lập mục III Sheet 00: chỉ tiêu Lợi nhuận sau thuế.', TOL_TY);
  FS99Q_addSummaryCheck_(add, summary, 'Tổng Thuế TNDN', srcCitTy,
    'Thuế TNDN', 'Sửa module lập mục III Sheet 00: chỉ tiêu Tổng Thuế TNDN.', TOL_TY);
  FS99Q_addSummaryCheck_(add, summary, 'Tổng VAT phải nộp', srcVatTy,
    'VAT phải nộp', 'Sửa module lập mục III Sheet 00: chỉ tiêu Tổng VAT phải nộp.', TOL_TY);

  // ================================================================
  // TẦNG 4 - KẾT LUẬN VÀ MỤC 14
  // ================================================================
  ...
  if (failRows.length) {
    ...
    rootCause = 'Lỗi đầu tiên nằm tại ' + first[0] + ': ' + first[1] + '. ' + first[7];
    results.push(['KẾT LUẬN', 'Vị trí sửa đầu tiên', first[2], first[3], first[4], 'FAIL',
      'Sửa từ tầng thấp lên tầng cao để tránh FAIL dây chuyền.', rootCause]);
  } else {
    rootCause = 'Các nguồn chi tiết, Sheet 04A và Sheet 00 đều khớp. Mục 14 chỉ là chỉ tiêu tham chiếu phạm vi.';
...
function FS99Q_addAnnualSourceCheck_(add, annual, label, sourceValue, shortName, tol) {
  ...
  add('TẦNG 2 - 04A', label + ' = nguồn Sheet 04', actual, sourceValue,
    'Đơn vị tỷ đồng.',
    'Sửa hàm tổng hợp theo năm tại dòng ' + shortName + ' của Sheet 04A.', tol);
}
...
function FS99Q_annualValue_(annual, label) {
  ...
  if (!(key in annual)) return 0;
  return FS99Q_num_(annual[key]);
}
...
function FS99Q_detectHeader_(sheet, requiredNames) {
  ...
  if (!best || best.score === 0) {
    throw new Error('Không nhận diện được dòng tiêu đề tại sheet ' + sheet.getName() + '.');
  }
  return best;
}
